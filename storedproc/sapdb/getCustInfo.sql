...
OUT C_Email varchar(50), OUT C_Data varchar(500), OUT C_BirthDate char(8)) AS
  VAR C_Addr_ID fixed(10,0); Addr_CO_ID fixed(4,0); b_date date;
BEGIN
SELECT c_id, c_passwd, c_fname, c_lname, c_phone, c_email, 
  c_addr_id, c_data, c_birthdate INTO :C_ID, :C_PassWD, :C_FName, :C_LName, 
  :C_Phone, :C_Email, :C_Addr_ID, :C_Data, :b_date
FROM dbt.CUSTOMER
WHERE c_uname = :C_UName;
set C_BirthDate = b_date;
...
INTO :Addr_Street1, :Addr_Street2, :Addr_City, :Addr_State, :Addr_Zip,
     :Addr_CO_ID
FROM dbt.address
WHERE addr_id = :C_Addr_ID;
SELECT co_name INTO :Co_Name from dbt.country WHERE co_id=:Addr_CO_ID;
END;;
/
/
