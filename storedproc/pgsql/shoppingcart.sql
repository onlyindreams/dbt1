...
-- declare sc_id and itemcount 
  ...
  BEGIN
    ...
    RAISE NOTICE ''_sc_id=%, _add_flag=%, _itemcount=%, _i_id=%, _pp_i_id=%'', _sc_id, _add_flag, _itemcount, _i_id, _pp_i_id;
    OPEN refcur FOR SELECT * FROM ARGshopping_cart;
    FETCH refcur
    INTO i_id1, qty1, i_id2, qty2, i_id3, qty3, i_id4, qty4,
         i_id5, qty5, i_id6, qty6, i_id7, qty7, i_id8, qty8,
         i_id9, qty9, i_id10, qty10, i_id11, qty11, i_id12, qty12,
         i_id13, qty13, i_id14, qty14, i_id15, qty15, i_id16, qty16,
         i_id17, qty17, i_id18, qty18, i_id19, qty19, i_id20, qty20;
    CLOSE refcur;
    IF NOT FOUND THEN
      DROP TABLE ARGshopping_cart;
      RETURN;
    END IF;

    DROP TABLE ARGshopping_cart;
    -- we have only three situations:
    -- from HO, BS, NP, BR, SRQ, SRS to SC, view cart the first time
    ...
    IF ( _add_flag=1 OR ( _add_flag=0 AND _itemcount=0 )) THEN
