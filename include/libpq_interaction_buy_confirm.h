/*
 * pgsql_interaction_buy_confirm.h
 *
...
#ifndef _LIBPQ_INTERACTION_BUY_CONFIRM_H
#define _LIBPQ_INTERACTION_BUY_CONFIRM_H

#include <libpq_interaction.h>

#define STMT_BUYCONF \
  "SELECT * FROM buy_confirm( '%s',%lld,'%s','%s',%lld,%lld," \
                             "%d,'%s','%s'," \
                             "'%s','%s','%s','%s','%s','%s' )" \
  " AS l( " \
...
int execute_buy_confirm(struct db_context_t *,struct buy_confirm_t *);

#endif /* _LIBPQ_INTERACTION_BUY_CONFIRM_H */
